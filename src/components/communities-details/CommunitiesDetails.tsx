...
const CommunitiesDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      axios.get(`http://localhost:3000/publication/comments/${id}/${userId}`)
        .then(response => {
      ...
      axios.get(`http://localhost:3000/publication/${id}`)
        .then(response => {
      ...
      <IonCard key={comment.id} className={`comment-level-${level}`}>
        <IonCardHeader>
          <IonChip className="comment-chip">
            ...
            <IonLabel>{comment.user.name} {comment.user.surname}</IonLabel>
          </IonChip>
          <IonCardSubtitle>{new Date(comment.created).toLocaleString()}</IonCardSubtitle>
        </IonCardHeader>
        <IonCardContent>
          {comment.content}
          <IonButton fill="clear" size="small" onClick={() => { setReplyToComment(comment); setShowDialog(true); }}>
            <IonIcon icon={chatboxOutline} slot="start" />
            Reply
          </IonButton>
          {comment.comments && comment.comments.length > 0 && renderComments(comment.comments, level + 1)}
        </IonCardContent>
      </IonCard>
    ));
  };
    ...
    <>
      ...
      <IonContent className="content-with-extra-padding">
        ...
        <Dialog open={showDialog} onClose={() => { setShowDialog(false); setReplyToComment(null); }} fullWidth maxWidth="sm">
          ...
          <DialogActions>
            <Button onClick={() => { setShowDialog(false); setReplyToComment(null); }} color="primary">
              Cancel
            </Button>
