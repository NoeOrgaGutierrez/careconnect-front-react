...
const UserInformation: React.FC<{ name: string }> = ({ name }) => {
	...
	useEffect(() => {
		...
		if (memberId) {
			axios
				.get(`http://34.116.158.34/user/${memberId}`)
				.then((response) => {
			...
			axios
				.get(`http://34.116.158.34/user/latest-comments/${memberId}`)
				.then((response) => {
			...
			axios
				.get(`http://34.116.158.34/user/pinned-blogs/${memberId}`)
				.then((response) => {
			...
			axios
				.get(`http://34.116.158.34/valoration/user/${memberId}`)
				.then((response) => {
	...
	const handleAssociationClick = () => {
		...
		if (memberId) {
			axios
				.get(`http://34.116.158.34/user-association/user/${memberId}`)
				.then((response) => {
